...
- **View Transitions** ⚡ v16/React 19.2 신규: App Router 라우트 이동 시 자동으로 View Transition 애니메이션 활성화. 루트 레이아웃에 ViewTransition 래퍼 불필요 — 네비게이션이 자동으로 전환 처리.
...
- **useEffectEvent** (React 19.2): Effect에서 비반응성 로직 분리 — 의존성 배열에 추가하지 않아도 되는 이벤트 핸들러 추출
- **Activity** (React 19.2): 상태를 유지하면서 백그라운드 UI를 비활성화/활성화 — 탭 전환, 사전 렌더링에 활용
