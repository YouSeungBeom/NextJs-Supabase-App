...
- Tailwind CSS v4 유틸리티 클래스만 사용
  - CSS 진입점: globals.css에 `@import "tailwindcss"` (구버전 `@tailwind base/components/utilities` 사용 금지)
  - 설정 파일 없음: 커스텀 테마는 globals.css 내 `@theme { }` 블록으로 직접 정의
  - tailwind.config.js/ts 참조 및 생성 금지
